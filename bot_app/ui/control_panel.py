...
from bot_app.ui.common import send_error, send_success, send_info, send_warning
...
def build_home_embed(guild: discord.Guild, logger, settings, db, stats=None) -> discord.Embed:
    ...
    e.set_footer(text="Loger Bot v4.0 | Используйте меню снизу для настройки")
    return e
...
class MainMenuSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            if val == "logger_cfg":
                if not logger: 
                    return await send_warning(interaction, "Логгер должен быть запущен для настройки (или просто включите его).", ephemeral=True)
            ...
            traceback.print_exc()
            await send_error(interaction, f"Ошибка меню: {e}", ephemeral=True)
...
class ControlPanelView(discord.ui.View):
    ...
    async def btn_start(self, interaction: discord.Interaction, _):
        ...
        try:
            ...
            await send_error(interaction, f"Не удалось запустить: {e}", ephemeral=True)
    ...
    async def btn_stop(self, interaction: discord.Interaction, _):
        ...
        try:
            ...
            await send_error(interaction, f"Ошибка остановки: {e}", ephemeral=True)
    ...
    async def btn_upload(self, interaction: discord.Interaction, _):
        logger = self.app.loggers.get(self.guild_id)
        if not logger:
            return await send_warning(interaction, "Логгер выключен.", ephemeral=True)
        ...
        try:
            ...
            await send_success(interaction, "Принудительная выгрузка завершена.", ephemeral=True)
        except Exception as e:
            await send_error(interaction, f"Ошибка выгрузки: {e}", ephemeral=True)
    ...
    async def btn_archives(self, interaction: discord.Interaction, _):
        from bot_app.ui.archive_menu import ArchiveView
        try:
            view = ArchiveView(self.guild_id)
            if not interaction.response.is_done():
                await interaction.response.send_message("📦 **Архивы Логов**", view=view, ephemeral=True)
            else:
                await interaction.followup.send("📦 **Архивы Логов**", view=view, ephemeral=True)
        except Exception as e:
            await send_error(interaction, f"Ошибка меню архивов: {e}", ephemeral=True)
