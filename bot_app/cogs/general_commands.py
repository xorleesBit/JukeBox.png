...
from bot_app.features.news_generator import generate_daily_news_json
...
class GeneralCommands(commands.Cog):
    ...
    async def cmd_news(self, ctx):
        status_msg = await ctx.send(embed=discord.Embed(description="📰 Редакция работает... (AI пишет)", color=discord.Color.blue()))
        try:
            data = await generate_daily_news_json(ctx.guild.id, ctx.guild.name)
            ...
            if "error" in data:
                ...
                e.add_field(name="👑 Герой Дня", value=hero, inline=True)
                e.add_field(name="💬 Цитата Дня", value=f"*{quote}*", inline=True)
                
                e.set_footer(text=f"Server Daily News | {ctx.guild.name}")
    ...
    async def cmd_profile(self, ctx, member: discord.Member = None):
        ...
        if not u_data:
            return await ctx.send(embed=discord.Embed(description="Профиль не найден.", color=discord.Color.red()))
        ...
        if p_data:
            ...
            if interests:
                e.add_field(name="💡 Интересы", value=", ".join(interests)[:1024], inline=True)
                
            games = p_data.get('games', [])
            if games:
                e.add_field(name="🎮 Игры", value=", ".join(games)[:1024], inline=True)
            ...
            if relations:
                # Try to format mentions
                rel_list = []
                for k, v in relations.items():
                    # If k is ID
                    if k.isdigit():
                        rel_list.append(f"<@{k}>: {v}")
                    else:
                        rel_list.append(f"**{k}**: {v}")
    ...
    async def cmd_update_achs(self, ctx):
        msg = await ctx.send(embed=discord.Embed(description="🏆 Раздаем ачивки...", color=discord.Color.blue()))
        mgr = ProfileManager(self.db, ctx.guild.id)
        res = await mgr.update_achievements(ctx.guild.name)
